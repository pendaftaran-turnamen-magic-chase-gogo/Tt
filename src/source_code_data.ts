...
function generateLocalQRString(qrisRaw: string, amount: number): string {
    ...
    if (qris.length > 4) { qris = qris.slice(0, -4); }
    const pointOfInitiation = qris.substring(4, 10);
    if (pointOfInitiation === '010211') { qris = qris.substring(0, 4) + '010212' + qris.substring(10); }
    ...
    if (idx53 !== -1) { const splitIdx = idx53 + currencyTag.length; qris = qris.substring(0, splitIdx) + tag54 + qris.substring(splitIdx); } 
    else { const countryTag = '5802ID'; const idx58 = qris.indexOf(countryTag); if (idx58 !== -1) { qris = qris.substring(0, idx58) + tag54 + qris.substring(idx58); } else { qris += tag54; } }
    qris += '6304';
    qris += crc16(qris);
    return qris;
}
...
export async function fetchQrisImage(qrisStatis: string, amount: number): Promise<string | null> {
  try {
    const response = await fetch("https://qrisku.my.id/api", {
      ...
      body: JSON.stringify({ amount: amount.toString(), qris_statis: qrisStatis })
    });
    if (!response.ok) throw new Error(\`HTTP \${response.status}\`);
    const data = await response.json();
    if (data.status === 'success' && data.qris_base64) { return \`data:image/png;base64,\${data.qris_base64}\`; }
...
const App: React.FC = () => {
  ...
  const setProof = (id: string, proofUrl: string) => {
      ...
      update(ref(db, path), { proofUrl: proofUrl, status: 'paid' });
  };
