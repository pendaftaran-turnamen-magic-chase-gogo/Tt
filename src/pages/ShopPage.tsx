...
import { ShoppingCart, Plus, Minus, Send, MapPin, X, Check, Wallet, QrCode, Camera, ShoppingBag, ChevronUp, Loader2, Clock, Trash2, AlertTriangle, RefreshCw, PartyPopper, Ban, Search, ChevronDown, Globe, MessageCircle, Star, ChevronRight, Info, Edit3 } from 'lucide-react';
  ...
  return (
    <div className="min-h-screen bg-slate-100 relative">
      ...
      <div className="yakult-gradient px-6 pt-12 pb-24 rounded-b-[48px] text-white shadow-xl relative overflow-hidden z-20">
        ...
        <Link to="/admin" className="absolute top-6 right-6 bg-white/20 hover:bg-white/30 px-4 py-2 rounded-2xl text-xs font-bold transition-all backdrop-blur-md">Admin</Link>
      </div>
      ...
      {/* --- CONTENT SECTION (SEPARATED UI) --- */}
      {/* Curved top divider & White Background */}
      ...
      {/* --- FLOATING STATUS BAR (REAL-TIME UPDATE) --- */}
      ...
      {/* Overlay Status Pesanan & Cart Modal & Form Order Modal (Standard) */}
      {/* Keeping these standard modals brief as they haven't changed logic, just rendered conditionally */}
      {currentTxId && activeTx && isPopupOpen && (
        <div className="fixed inset-0 z-[100] bg-slate-900/60 backdrop-blur-md flex items-center justify-center p-6 animate-in fade-in duration-300">
          <div className="bg-white w-full max-w-sm rounded-[48px] overflow-hidden shadow-2xl relative max-h-[90vh] overflow-y-auto">
            {/* Status Views (Pending/Confirmed/Rejected) same as before... */}
